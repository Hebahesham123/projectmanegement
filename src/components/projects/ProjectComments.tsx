    ...
    try {
      ...
      if (project?.owner_email) {
        ...
        const mail = buildEmail({
          accent: 'brand',
          eyebrow: 'Project comment',
          preheader: `${authorName} commented on "${project.name}"`,
          heading: `New comment on: ${project.name}`,
          recipientName: project.owner_name ?? undefined,
          intro: `${authorName} left a new comment on your project.`,
          facts: [
            { label: 'Project', value: project.name },
            { label: 'Commented by', value: authorName },
          ],
          message: body.trim(),
          cta: { label: 'View project', href: `${origin}${href}` },
        });
        if (project.created_by && project.created_by !== user.id) {
          notify({
            ...
            email: {
              ...
              subject: `[NS Project Tracker] New comment on "${project.name}"`,
              html: mail.html,
              text: mail.text,
            },
          });
